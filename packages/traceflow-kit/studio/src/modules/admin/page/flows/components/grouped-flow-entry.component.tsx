  ...
  return (
    <>
      <div className="p-4">
        ...
        <button className={`${GROUPED_FLOW_LINK_CLASS} mt-3`} onClick={() => selectSpan(requestSpan ?? span, 'input')}>
      ...
      {data.preconditions.length ? (
        <div className="border-t border-zinc-200 bg-transparent px-4 py-3 dark:border-zinc-700">
          ...
          <GroupedFlowStepsComponent nodes={data.preconditions} query={data.query} matchedIds={data.matchedIds} compact />
        </div>
      ) : null}
    </>
  );
